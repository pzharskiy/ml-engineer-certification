...
pipeline {
    ...
    stages {
        ...
        stage('Notify started') {
            steps {
                notifySlack("started (${symbol_dollar}{buildCause()})", 'lightskyblue', this)
                notifyEmail("started (${symbol_dollar}{buildCause()})", this)
            }
        }
        ...
        stage('deploy to CT') {
            ...
            steps {
                ...
                dir('ml-engineer-certification-package') {
        ...
        stage('Release') {
            ...
            stages {
                ...
                stage('Make a tag and push') {
                    steps {
                        shellScript "git tag ${symbol_dollar}{mavenVersion()}"
...
def notifyEmail(String buildMessage, def script) {
    ...
    script.emailext(
            subject: "'${symbol_dollar}{env.JOB_NAME} [${symbol_dollar}{env.BUILD_NUMBER}]' ${symbol_dollar}{buildMessage} ",
            to: params.EMAILS,
            body: """
'${symbol_dollar}{env.JOB_NAME} [${symbol_dollar}{env.BUILD_NUMBER}]' ${symbol_dollar}{buildMessage} :
Check console output at "${symbol_dollar}{env.JOB_NAME} [${symbol_dollar}{env.BUILD_NUMBER}]"
